...
if (!isset($_SESSION['user_id'])) {
    ...
    header("Location: ../dashboard/tiktok.php");
    exit();
}
...
$stmt = $conn->prepare("SELECT id FROM tiktok_video_views WHERE user_id = ? AND video_id = ?");
...
try {
    // Record the view
    $stmt = $conn->prepare("INSERT INTO tiktok_video_views (user_id, video_id) VALUES (?, ?)");
    $stmt->bind_param("ii", $user_id, $video_id);
    $stmt->execute();
    $stmt->close();
    ...
    // Log the transaction
    $desc = "TikTok video watch reward";
